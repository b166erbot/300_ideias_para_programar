...
cores = (('verde', 'green'), ('água', 'blue'), ('carbônico', 'grey_0'),
         ('ar', 'blue_1'), ('solar', 'yellow'), ('planta', 'green'),
         ('raízes', 'rosy_brown'), ('caule', 'white'), ('folhas', 'green'),
         ('energia', 'yellow'), ('sol', 'yellow'),
         ('hidrogênio', 'blue'), ('solo', 'rosy_brown'))


def colorir(texto):
    texto = texto.split()
    for palavra in texto:
        for palavra2, cor in cores:
            if palavra.strip(',.') == palavra2:
                texto[texto.index(palavra)] = f'{fg(cor)}{palavra}{attr(0)}'
    return ' '.join(texto)
...
class Constructor:
    ...
    texto = tuple(colorir(a) for a in texto)
